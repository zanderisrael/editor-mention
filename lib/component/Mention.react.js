...
var addMentionKeyToCharacterList = function addMentionKeyToCharacterList(characterList, startIndex, length, mentionKey) {
  for (var index = 0; index < length; index++) {
    var characterMetadata = characterList.get(startIndex + index);
    var meta = characterMetadata.set("entity", mentionKey);
...
var addMentionsToContentBlock = function addMentionsToContentBlock(contentState, block, trigger, suggestions) {
  ...
  var regex = (0, _getRegExp2['default'])(trigger, suggestions, true, true);
  ...
  for (var _iterator = matches, _isArray = Array.isArray(_iterator), _i = 0, _iterator = _isArray ? _iterator : _iterator[Symbol.iterator]();;) {
    ...
    var mentionKey = contentState.getLastCreatedEntityKey();
...
var Mention = function (_React$Component) {
  ...
  Mention.prototype.render = function render() {
    ...
    var defaultValueState = defaultValue && _draftJs.EditorState.createWithContent(addMentionsToContentState(typeof defaultValue === 'string' ? _draftJs.ContentState.createFromText(defaultValue) : defaultValue, this.props.prefix, suggestions), this._decorator);
